...
import {
  ...
  ar4055,
  arFunctional,
} from "../data/arunachal-pradesh/police";
import { formatCrore } from "../lib/money";

export function ArPolicePage() {
  const hero = pickAmount(arFunctional, AR_HEADLINE_YEAR, AR_HEADLINE_SERIES);
  const run = pickAmount(ar2055, AR_HEADLINE_YEAR, AR_HEADLINE_SERIES);
  const cap = pickAmount(ar4055, AR_HEADLINE_YEAR, AR_HEADLINE_SERIES);
  const idx = getIndexRow("arunachal-pradesh");
  if (!hero || !run || !cap || !idx) throw new Error("Missing Arunachal Pradesh gold figures");
  ...
  return (
    <article>
        ...
        stops={[
          {
            id: "book",
            label: "Book",
            detail: "Arunachal Pradesh Annual Financial Statement 2026-27",
          },
      ...
      <section className="index-slip mt-10">
        <p className="mt-3 max-w-2xl text-sm text-ink/75">
          A research summary lists Arunachal Pradesh police at ₹{idx.be2526.toLocaleString("en-IN")}{" "}
