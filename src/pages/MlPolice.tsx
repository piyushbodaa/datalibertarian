...
import {
  ...
  ml4055,
  mlFunctional,
} from "../data/meghalaya/police";
import { formatCrore } from "../lib/money";

export function MlPolicePage() {
  const hero = pickAmount(mlFunctional, ML_HEADLINE_YEAR, ML_HEADLINE_SERIES);
  const run = pickAmount(ml2055, ML_HEADLINE_YEAR, ML_HEADLINE_SERIES);
  const cap = pickAmount(ml4055, ML_HEADLINE_YEAR, ML_HEADLINE_SERIES);
  const idx = getIndexRow("meghalaya");
  if (!hero || !run || !cap || !idx) throw new Error("Missing Meghalaya gold figures");

  return (
    <article>
      <h1 className="mt-3 font-display text-4xl font-semibold tracking-tight">Meghalaya Police</h1>
      <p className="mt-4 max-w-2xl text-ink">
        ...
        not fire, not police housing under 2216.
      </p>
      ...
      <TraceRail
        stops={[
          { id: "book", label: "Book", detail: "Meghalaya Annual Financial Statement 2026-27" },
      ...
      <section className="index-slip mt-10">
        <p className="mt-3 max-w-2xl text-sm text-ink/75">
          A research summary lists Meghalaya police at ₹{idx.be2526.toLocaleString("en-IN")} crore
      ...
      <section className="mt-12 text-sm text-ink/70">
        ...
        <p className="mt-6">
          ...
          {" · "}
          <Link to="/compare?left=maharashtra&right=meghalaya">Compare with Maharashtra</Link>
          {" · "}
