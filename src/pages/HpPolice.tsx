...
import {
  ...
  hpDemand07,
  hpFunctional,
} from "../data/himachal-pradesh/police";
import { formatCrore } from "../lib/money";

export function HpPolicePage() {
  const hero = pickAmount(hpFunctional, HP_HEADLINE_YEAR, HP_HEADLINE_SERIES);
  const run = pickAmount(hp2055, HP_HEADLINE_YEAR, HP_HEADLINE_SERIES);
  const cap = pickAmount(hp4055, HP_HEADLINE_YEAR, HP_HEADLINE_SERIES);
  const mixed = pickAmount(hpDemand07, HP_HEADLINE_YEAR, HP_HEADLINE_SERIES);
  const idx = getIndexRow("himachal-pradesh");
  if (!hero || !run || !cap || !mixed || !idx) throw new Error("Missing Himachal Pradesh gold figures");
  ...
  return (
    <article>
      ...
      <p className="mt-4 max-w-2xl text-ink">
        ...
        not the whole Police and Allied grant.
      </p>
      ...
      <section className="carbon-sheet mt-10 px-4 py-6 sm:px-6">
        <h2 className="mt-2 font-display text-xl font-semibold">
          The Police and Allied grant is not only police
        </h2>
        <p className="mt-3 max-w-2xl text-sm text-ink/75">
          Demand 07 also carries jails, home guards, and housing. That bigger number is not Police.
          The Police number above is running costs plus buildings and gear.
        </p>
        <p className="mt-4 text-ink/80">
          Police and Allied grant, {mixed.fiscalYear} plan: <Money money={mixed} size="row" />
        </p>
      </section>
        ...
        stops={[
          {
            id: "book",
            label: "Book",
            detail: "Himachal Pradesh Annual Financial Statement 2026-27, Consolidated Fund",
          },
      ...
      <section className="index-slip mt-10">
        <p className="mt-3 max-w-2xl text-sm text-ink/75">
          A research summary lists Himachal Pradesh police at ₹{idx.be2526.toLocaleString("en-IN")}{" "}
          crore for 2025-26. That is not this official 2026-27 plan of ₹{formatCrore(hero.crore)}{" "}
          crore.
        </p>
      </section>
