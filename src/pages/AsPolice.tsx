...
import {
  ...
  as4055,
  asFunctional,
} from "../data/assam/police";
import { formatCrore } from "../lib/money";

export function AsPolicePage() {
  const hero = pickAmount(asFunctional, AS_HEADLINE_YEAR, AS_HEADLINE_SERIES);
  const run = pickAmount(as2055, AS_HEADLINE_YEAR, AS_HEADLINE_SERIES);
  const cap = pickAmount(as4055, AS_HEADLINE_YEAR, AS_HEADLINE_SERIES);
  const idx = getIndexRow("assam");
  if (!hero || !run || !cap || !idx) throw new Error("Missing Assam gold figures");

  return (
    <article>
      <h1 className="mt-3 font-display text-4xl font-semibold tracking-tight">Assam Police</h1>
      ...
      <PrintedColumns
        run={as2055}
        cap={as4055}
        caption="Spent, plan, updated plan, and next plan. Printed in lakhs; shown in crore. Running costs add the Police grant and one smaller police line on another grant."
      />
      <HeadSplit run={run} cap={cap} />

      <section className="carbon-sheet mt-10 px-4 py-6 sm:px-6">
        <h2 className="mt-2 font-display text-xl font-semibold">The Police grant is not the only police line</h2>
        <p className="mt-3 max-w-2xl text-sm text-ink/75">
          Grant 14 is the Police demand — running costs plus buildings. A smaller running-costs line
          also sits on Grant 19. Both are in the number above. Jails are a different grant.
        </p>
      </section>
      ...
      <TraceRail
        stops={[
          { id: "book", label: "Book", detail: "Assam Annual Financial Statement 2026-27, Statement B" },
      ...
      <section className="index-slip mt-10">
        <p className="mt-3 max-w-2xl text-sm text-ink/75">
          A research summary lists Assam police at ₹{idx.be2526.toLocaleString("en-IN")} crore for
