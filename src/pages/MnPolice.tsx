...
import {
  ...
  mnDemand07,
  mnFunctional,
} from "../data/manipur/police";
import { formatCrore } from "../lib/money";

export function MnPolicePage() {
  const hero = pickAmount(mnFunctional, MN_HEADLINE_YEAR, MN_HEADLINE_SERIES);
  const run = pickAmount(mn2055, MN_HEADLINE_YEAR, MN_HEADLINE_SERIES);
  const cap = pickAmount(mn4055, MN_HEADLINE_YEAR, MN_HEADLINE_SERIES);
  const mixed = pickAmount(mnDemand07, MN_HEADLINE_YEAR, MN_HEADLINE_SERIES);
  const idx = getIndexRow("manipur");
  if (!hero || !run || !cap || !mixed || !idx) throw new Error("Missing Manipur gold figures");

  return (
    <article>
      <h1 className="mt-3 font-display text-4xl font-semibold tracking-tight">Manipur Police</h1>
      <p className="mt-4 max-w-2xl text-ink">
        ...
        not housing, not the whole Police demand.
      </p>
      ...
      <section className="carbon-sheet mt-10 px-4 py-6 sm:px-6">
        <h2 className="mt-2 font-display text-xl font-semibold">The Police demand is not only 2055</h2>
        <p className="mt-3 max-w-2xl text-sm text-ink/75">
          Demand 07 also carries housing, social security, and census lines. That bigger number is
          not Police. The Police number above is running costs plus buildings and gear from the
          Annual Financial Statement.
        </p>
        <p className="mt-4 text-ink/80">
          Police demand, {mixed.fiscalYear} plan: <Money money={mixed} size="row" />
        </p>
      </section>
      ...
      <TraceRail
        stops={[
          { id: "book", label: "Book", detail: "Manipur Annual Financial Statement 2026-27" },
      ...
      <section className="index-slip mt-10">
        <p className="mt-3 max-w-2xl text-sm text-ink/75">
          A research summary lists Manipur police at ₹{idx.be2526.toLocaleString("en-IN")} crore for
      ...
      <section className="mt-12 text-sm text-ink/70">
        ...
        <p className="mt-6">
          ...
          {" · "}
          <Link to="/compare?left=maharashtra&right=manipur">Compare with Maharashtra</Link>
          {" · "}
