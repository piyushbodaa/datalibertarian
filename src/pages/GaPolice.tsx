...
import {
  ...
  gaDemand17,
  gaFunctional,
} from "../data/goa/police";
import { formatCrore } from "../lib/money";

export function GaPolicePage() {
  const hero = pickAmount(gaFunctional, GA_HEADLINE_YEAR, GA_HEADLINE_SERIES);
  const run = pickAmount(ga2055, GA_HEADLINE_YEAR, GA_HEADLINE_SERIES);
  const cap = pickAmount(ga4055, GA_HEADLINE_YEAR, GA_HEADLINE_SERIES);
  const mixed = pickAmount(gaDemand17, GA_HEADLINE_YEAR, GA_HEADLINE_SERIES);
  const idx = getIndexRow("goa");
  if (!hero || !run || !cap || !mixed || !idx) throw new Error("Missing Goa gold figures");

  return (
    <article>
      <h1 className="mt-3 font-display text-4xl font-semibold tracking-tight">Goa Police</h1>
      <p className="mt-4 max-w-2xl text-ink">
        ...
        not pensions, not the whole Police demand.
      </p>
      ...
      <section className="carbon-sheet mt-10 px-4 py-6 sm:px-6">
        <h2 className="mt-2 font-display text-xl font-semibold">The Police demand is not only the police line</h2>
        <p className="mt-3 max-w-2xl text-sm text-ink/75">
          Demand 17 also carries pensions and a road-transport slice. Forensic science sits on a
          different demand under the same police running-costs code. The number above is the
          statewide police line — running costs plus buildings — not that mixed demand total.
        </p>
      ...
      <TraceRail
        stops={[
          { id: "book", label: "Book", detail: "Goa Annual Financial Statement 2026-27" },
      ...
      <section className="index-slip mt-10">
        <p className="mt-3 max-w-2xl text-sm text-ink/75">
          A research summary lists Goa police at ₹{idx.be2526.toLocaleString("en-IN")} crore for
