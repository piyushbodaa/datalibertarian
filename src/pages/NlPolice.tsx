...
import {
  ...
  nl4055,
  nlFunctional,
} from "../data/nagaland/police";
import { formatCrore } from "../lib/money";

export function NlPolicePage() {
  const hero = pickAmount(nlFunctional, NL_HEADLINE_YEAR, NL_HEADLINE_SERIES);
  const run = pickAmount(nl2055, NL_HEADLINE_YEAR, NL_HEADLINE_SERIES);
  const cap = pickAmount(nl4055, NL_HEADLINE_YEAR, NL_HEADLINE_SERIES);
  const idx = getIndexRow("nagaland");
  if (!hero || !run || !cap || !idx) throw new Error("Missing Nagaland gold figures");

  return (
    <article>
      <h1 className="mt-3 font-display text-4xl font-semibold tracking-tight">Nagaland Police</h1>
      ...
      <TraceRail
        stops={[
          { id: "book", label: "Book", detail: "Nagaland Annual Financial Statement 2026-27" },
      ...
      <section className="index-slip mt-10">
        <p className="mt-3 max-w-2xl text-sm text-ink/75">
          A research summary lists Nagaland police at ₹{idx.be2526.toLocaleString("en-IN")} crore for
      ...
      <section className="mt-12 text-sm text-ink/70">
        ...
        <p className="mt-6">
          ...
          {" · "}
          <Link to="/compare?left=maharashtra&right=nagaland">Compare with Maharashtra</Link>
          {" · "}
