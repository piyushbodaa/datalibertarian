...
import {
  ...
  sk4055,
  skFunctional,
} from "../data/sikkim/police";
import { formatCrore } from "../lib/money";

export function SkPolicePage() {
  const hero = pickAmount(skFunctional, SK_HEADLINE_YEAR, SK_HEADLINE_SERIES);
  const run = pickAmount(sk2055, SK_HEADLINE_YEAR, SK_HEADLINE_SERIES);
  const cap = pickAmount(sk4055, SK_HEADLINE_YEAR, SK_HEADLINE_SERIES);
  const idx = getIndexRow("sikkim");
  if (!hero || !run || !cap || !idx) throw new Error("Missing Sikkim gold figures");

  return (
    <article>
      <PoliceOverview name="Sikkim" description="What the state budget set aside to run the police and to build or buy for them. Not jails, not a research-summary sector slice." hero={hero} run={run} cap={cap} runningLine={sk2055} capitalLine={sk4055} caption="Spent, plan, updated plan, and next plan. Printed in thousands; shown in crore." />
        ...
        stops={[
          {
            id: "book",
            label: "Book",
            detail: "Sikkim Annual Financial Statement 2026-27, Statement I",
          },
      ...
      <section className="index-slip mt-10">
        <p className="mt-3 max-w-2xl text-sm text-ink/75">
          A research summary lists Sikkim police at ₹{idx.be2526.toLocaleString("en-IN")} crore for
      ...
      <section className="mt-12 text-sm text-ink/70">
        ...
        <p className="mt-6">
          ...
          {" · "}
          <Link to="/compare?left=maharashtra&right=sikkim">Compare with Maharashtra</Link>
          {" · "}
