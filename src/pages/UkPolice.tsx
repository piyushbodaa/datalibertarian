...
import {
  ...
  ukFunctional,
  ukGrant10,
} from "../data/uttarakhand/police";
import { formatCrore } from "../lib/money";

export function UkPolicePage() {
  const hero = pickAmount(ukFunctional, UK_HEADLINE_YEAR, UK_HEADLINE_SERIES);
  const run = pickAmount(uk2055, UK_HEADLINE_YEAR, UK_HEADLINE_SERIES);
  const cap = pickAmount(uk4055, UK_HEADLINE_YEAR, UK_HEADLINE_SERIES);
  const mixed = pickAmount(ukGrant10, UK_HEADLINE_YEAR, UK_HEADLINE_SERIES);
  const idx = getIndexRow("uttarakhand");
  if (!hero || !run || !cap || !mixed || !idx) throw new Error("Missing Uttarakhand gold figures");

  return (
    <article>
      <h1 className="mt-3 font-display text-4xl font-semibold tracking-tight">Uttarakhand Police</h1>
      <p className="mt-4 max-w-2xl text-ink">
        ...
        not the whole Police and Jail grant.
      </p>
      ...
      <section className="carbon-sheet mt-10 px-4 py-6 sm:px-6">
        <h2 className="mt-2 font-display text-xl font-semibold">The Police and Jail grant is not only police</h2>
        <p className="mt-3 max-w-2xl text-sm text-ink/75">
          Grant 10 also carries jails and mixed public-works capital. That bigger number is not
          Police. The Police number above is running costs plus buildings and gear.
        </p>
        <p className="mt-4 text-ink/80">
          Police and Jail grant, {mixed.fiscalYear} plan: <Money money={mixed} size="row" />
        </p>
      </section>
        ...
        stops={[
          {
            id: "book",
            label: "Book",
            detail: "Uttarakhand Annual Financial Statement 2026-27, Volume 2 Part 1",
          },
      ...
      <section className="index-slip mt-10">
        <p className="mt-3 max-w-2xl text-sm text-ink/75">
          A research summary lists Uttarakhand police at ₹{idx.be2526.toLocaleString("en-IN")} crore
