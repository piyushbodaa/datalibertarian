...
import {
  ...
  brDemand22,
  brFunctional,
} from "../data/bihar/police";
import { formatCrore } from "../lib/money";

export function BrPolicePage() {
  const hero = pickAmount(brFunctional, BR_HEADLINE_YEAR, BR_HEADLINE_SERIES);
  const run = pickAmount(br2055, BR_HEADLINE_YEAR, BR_HEADLINE_SERIES);
  const cap = pickAmount(br4055, BR_HEADLINE_YEAR, BR_HEADLINE_SERIES);
  const mixed = pickAmount(brDemand22, BR_HEADLINE_YEAR, BR_HEADLINE_SERIES);
  const idx = getIndexRow("bihar");
  if (!hero || !run || !cap || !mixed || !idx) throw new Error("Missing Bihar gold figures");

  return (
    <article>
      <h1 className="mt-3 font-display text-4xl font-semibold tracking-tight">Bihar Police</h1>
      ...
      <section className="carbon-sheet mt-10 px-4 py-6 sm:px-6">
        ...
        <p className="mt-3 max-w-2xl text-sm text-ink/75">
          Demand 22 also carries jails and public works. That bigger number is not Police. The
          Police number above is running costs plus buildings and gear.
        </p>
        ...
        stops={[
          {
            id: "book",
            label: "Book",
            detail: "Bihar Annual Financial Statement 2026-27, Statement I",
          },
      ...
      <section className="index-slip mt-10">
        <p className="mt-3 max-w-2xl text-sm text-ink/75">
          A research summary lists Bihar police at ₹{idx.be2526.toLocaleString("en-IN")} crore for
      ...
      <section className="mt-12 text-sm text-ink/70">
        ...
        <p className="mt-6">
          ...
          {" · "}
          <Link to="/compare?left=maharashtra&right=bihar">Compare with Maharashtra</Link>
          {" · "}
