...
import {
  ...
  mz4055,
  mzFunctional,
} from "../data/mizoram/police";
import { formatCrore } from "../lib/money";

export function MzPolicePage() {
  const hero = pickAmount(mzFunctional, MZ_HEADLINE_YEAR, MZ_HEADLINE_SERIES);
  const run = pickAmount(mz2055, MZ_HEADLINE_YEAR, MZ_HEADLINE_SERIES);
  const cap = pickAmount(mz4055, MZ_HEADLINE_YEAR, MZ_HEADLINE_SERIES);
  const idx = getIndexRow("mizoram");
  if (!hero || !run || !cap || !idx) throw new Error("Missing Mizoram gold figures");

  return (
    <article>
      <h1 className="mt-3 font-display text-4xl font-semibold tracking-tight">Mizoram Police</h1>
      ...
      <TraceRail
        stops={[
          { id: "book", label: "Book", detail: "Mizoram Annual Financial Statement 2026-27" },
      ...
      <section className="index-slip mt-10">
        <p className="mt-3 max-w-2xl text-sm text-ink/75">
          A research summary lists Mizoram police at ₹{idx.be2526.toLocaleString("en-IN")} crore for
      ...
      <section className="mt-12 text-sm text-ink/70">
        ...
        <p className="mt-6">
          ...
          {" · "}
          <Link to="/compare?left=maharashtra&right=mizoram">Compare with Maharashtra</Link>
          {" · "}
