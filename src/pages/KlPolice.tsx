...
import {
  ...
  kl4055,
  klFunctional,
} from "../data/kerala/police";
import { formatCrore } from "../lib/money";

export function KlPolicePage() {
  const hero = pickAmount(klFunctional, KL_HEADLINE_YEAR, KL_HEADLINE_SERIES);
  const run = pickAmount(kl2055, KL_HEADLINE_YEAR, KL_HEADLINE_SERIES);
  const cap = pickAmount(kl4055, KL_HEADLINE_YEAR, KL_HEADLINE_SERIES);
  const idx = getIndexRow("kerala");
  if (!hero || !run || !cap || !idx) throw new Error("Missing Kerala gold figures");

  return (
    <article>
      <h1 className="mt-3 font-display text-4xl font-semibold tracking-tight">Kerala Police</h1>
      <p className="mt-4 max-w-2xl text-ink">
        ...
        not courts, not the vigilance office.
      </p>
      ...
      <PrintedColumns
        run={kl2055}
        cap={kl4055}
        caption="Spent, plan, updated plan, and next plan. Accounts printed in rupees; later columns in thousands; shown in crore."
      />
      ...
      <section className="carbon-sheet mt-10 px-4 py-6 sm:px-6">
        ...
        <p className="mt-3 max-w-2xl text-sm text-ink/75">
          Demand XII also carries vigilance. Jails sit on a different demand. Those bigger or
          neighbour numbers are not Police. The Police number above is running costs plus buildings
          and gear.
        </p>
      </section>
      ...
      <TraceRail
        stops={[
          { id: "book", label: "Book", detail: "Kerala Annual Financial Statement 2026-27" },
      ...
      <section className="index-slip mt-10">
        <p className="mt-3 max-w-2xl text-sm text-ink/75">
          A research summary lists Kerala police at ₹{idx.be2526.toLocaleString("en-IN")} crore for
