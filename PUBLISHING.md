...
Babuwatch (`/babuwatch`) is built in the same deployment by `npm run build:babuwatch` (Python 3, stdlib) from `babuwatch/`; see `babuwatch/README.md`. Copwatch India is merged fully under Babuwatch: `/copwatchindia/*` and `/babuwatch/copwatchindia/*` redirect permanently to `/babuwatch/*` (see `vercel.json`); the separate `copwatchindia` Vercel project and repository are not used by this site.
